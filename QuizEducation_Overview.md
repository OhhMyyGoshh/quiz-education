...
Express match routes từ trên xuống — static paths **phải** đứng trước param paths:
- `/classrooms/join` trước `/classrooms/:id`
- `/classrooms/enrolled` trước `/classrooms/:id`
- `/quizzes/public` phải đăng ký trước `/quizzes/:id` **trong server.ts**

### Security
- `password` và `refreshToken` có `select: false` — không bao giờ lộ trong response
- Student không thể xem `isCorrect` khi lấy quiz để làm bài
- `correctChoiceIndex` chỉ trả về sau nộp bài và chỉ khi `showAnswerAfter = true`
- `assignQuiz` kiểm tra `quiz.createdBy === teacherId` trước khi cho gán

### Soft Delete
- Classroom dùng `isActive: false` thay vì xoá khỏi DB
- Giữ lại lịch sử submission ngay cả khi lớp bị xoá

### Environment Variables
```env
PORT=3000
MONGO_URI=mongodb://localhost:27017/quiz_education
...
```bash
npm run seed:admin
# Email: admin@quiz.com / Password: admin123456
# ⚠️ Đổi mật khẩu trước khi deploy production
```
